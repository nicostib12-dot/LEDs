...
; MICRO: PIC18F4550
; COMPILADOR: PIC-AS (XC8 Assembler)
; MÉTODO: Timer0 modo 16 bits (Polling)
; SOLO SE CONFIGURA RB0 COMO SALIDA
;========================================================

        PROCESSOR 18F4550 ; Indica el microcontrolador
...
# 11 "LEDBlink1test.asm" 2

;========================================================
; VARIABLES EN RAM (ACCESS BANK)
;========================================================
; udata_acs = sección de datos en RAM de acceso rápido
; Las variables aquí pueden usarse sin cambiar banco
;========================================================

PSECT udata_acs

CONTADOR: DS 1 ; Cuenta desbordamientos (cada 50 ms)
ESTADO: DS 1 ; 0 = LED apagado, 1 = LED encendido

;========================================================
; VECTOR DE RESET (MEMORIA FLASH)
;========================================================
; resetVec = sección donde el micro comienza a ejecutar
; reloc=2 es obligatorio en PIC18 (instrucciones de 2 bytes)
;========================================================

PSECT resetVec,class=CODE,reloc=2

resetVec:
        GOTO ((SSPSTAT) and 0FFh), 3, a ; Al encender, saltar al inicio

;========================================================
; CÓDIGO PRINCIPAL (MEMORIA FLASH)
;========================================================

PSECT code

((SSPSTAT) and 0FFh), 3, a:

    ;--------------------------------------------
    ; Configurar ((PORTB) and 0FFh), 0, a como salida digital
    ;--------------------------------------------
    ; TRISB bit 0 = 0 ? salida
    ; LATB bit 0 = 0 ? LED inicia apagado
    ;--------------------------------------------
...
MAIN:
    ...
    ;--------------------------------------------
    ; Esperar desbordamiento del Timer0
    ;--------------------------------------------

        BTFSS INTCON,((INTCON) and 0FFh), 2, a ; ¿Se activó la bandera?
        GOTO MAIN ; Si no, seguir esperando

    ;--------------------------------------------
    ; Limpiar bandera de overflow
    ;--------------------------------------------

        BCF INTCON,((INTCON) and 0FFh), 2, a

    ;--------------------------------------------
    ; Recargar Timer0 para otros 50 ms
    ;--------------------------------------------
    ...
    ;--------------------------------------------
    ; Incrementar contador (cada 50 ms)
    ;--------------------------------------------

        INCF CONTADOR,F

    ;--------------------------------------------
    ; Verificar estado actual del LED
    ;--------------------------------------------
    ; Si ESTADO = 0 ? LED apagado
    ; Si ESTADO = 1 ? LED encendido
    ;--------------------------------------------
...
;========================================================
; CASO: LED ENCENDIDO ? Esperar 2 segundos
; 40 × 50 ms = 2 s
;========================================================
...
LED_ENCENDIDO:
        ...
        MOVLW 40
        SUBWF CONTADOR,W
        BTFSS STATUS,Z ; ¿Ya llegó a 40?
        GOTO MAIN ; Si no, seguir esperando

        ; Ya pasaron 2 segundos

        BCF LATB,0 ; Apagar LED
        CLRF CONTADOR ; Reiniciar contador
        CLRF ESTADO ; Estado = 0 (apagado)
        GOTO MAIN

;========================================================
; CASO: LED APAGADO ? Esperar 1 segundo
; 20 × 50 ms = 1 s
;========================================================
...
LED_APAGADO:
        ...
        CLRF CONTADOR ; Reiniciar contador
        MOVLW 1
        MOVWF ESTADO ; Estado = 1 (encendido)
        GOTO MAIN
